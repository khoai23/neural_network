...
def createWritingData(treeList, alignmentList):
	allExamples = []
	
	for caseIdx in range(len(treeList)):
		tree = treeList[caseIdx]
		alignment = alignmentList[caseIdx]
		switchList = assignAlignment(alignment, tree)
		example = alignmentToTrainingExample(tree, switchList)
...
if __name__ == "__main__":
	...
	parser.add_argument('-x','--output_extension', type=str, default="txt", help='output file mode (txt|arff), default txt')
	parser.add_argument('-e','--embedding_word', type=str, help='embedding word file')
	parser.add_argument('--extension', type=str, default="parser", help='file extension for parser, default parser')
	parser.add_argument('--default_word', type=str, default="<unk>", help='key in embedding_word standing for unknown word')
	parser.add_argument('--full_load', action='store_true', help='get all vector for words in embedding_word file. Not recommended for large word file')
	...
	if(args.mode == 'stanford'):
		treeList, alignmentList = runStanfordParser(args.inputdir, args.extension)
		print("Done for @StanfordParser, time passed %.2fs" % (time.time() - timer))
	elif(args.mode == 'conll'):
		treeList, alignmentList = runConllParser(args.inputdir, args.extension)
	...
	allExamples = createWritingData(treeList, alignmentList)
	...
	if(args.output_extension == 'txt'):
		...
		raise argparse.ArgumentTypeError("Incorect output_extension, must be txt|arff")
	# Execute script