...
#			orig_line = line
	...
	for block in json_data:
		input_batch = block[input_field]
		input_batch = [cleaner(line) for line in input_batch]
		if(not infer_mode):
			# data is tuple of (input, correct_output)
			output_batch = block[output_field]
			full_data_list.extend(zip(input_batch, output_batch))
		else:
			# data is single cleaned line
			full_data_list.extend(input_batch)
	if(infer_mode):
		return json_data, full_data_list
	else:
		return full_data_list

def readCSVData(file_path, input_field=0, output_field=1, remove_first_row=True, infer_mode=False):
	...
	if("{:s}" in test_file):
		...
		with io.open(os.path.join(folder, test_folder, test_file), "r", encoding="utf-8") as read_file:
			...
			for test_line, answer_line in zip(read_file, read_file):
				...
				eval_data_list.append( (clean(test_line), score) )
...
def tryReadDataFromPath(data_config):
	...
	if(isinstance(path, (list, tuple, set)) and path_type=="files"):
		...
		print("Path {:s} is directory, use splitted text(SA-2016) format".format(path))
	...
	elif(os.path.isfile(path) and path_type == "csv"):
		print("Path {:s} is single file, use CSV reading format".format(path))
		return readCSVData(path, **data_config)
	else:
...
def constructParser():
	...
	parser.add_argument('--model_structure', type=str, default='attention', choices=["attention", "attention_extended", "attention_extended_v2", "attention_multimodal", "sum", "self_attention"], help='The model structure to be used in sentiment model. Default attention')
...
def createModel(args, dataset):
	...
	if(args.model_structure == "attention"):
		...
		model.buildSession(words, embeddings, default_word_idx, cell_size=args.cell_size, additional_words=additional_words, gpu_allow_growth=not args.gpu_disable_allow_growth, optimizer=args.optimizer)
	elif(args.model_structure == "self_attention"):
		model = model_lib.SentimentSelfAttention(args.save_file, args.export_dir, batch_size=args.batch_size, debug=args.debug, shuffle_batch=args.shuffle_batch)
		model.buildSession(words, embeddings, default_word_idx, cell_size=args.cell_size, additional_words=additional_words, gpu_allow_growth=not args.gpu_disable_allow_growth, optimizer=args.optimizer)
	else:
