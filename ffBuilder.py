...
def createDecoder(settingDict):
	...
	if(correctResult is not None):
		...
		with tf.control_dependency([tf.assert_none_equal(decoderTrainingInput, correctResult), tf.assert_equal(tf.shape(decoderTrainingInput), tf.shape(correctResult))]):
