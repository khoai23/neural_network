...
def createEmbeddingCouplingFromFile(args):
	srcDict = getEmbeddingFromFile(os.path.join(args.directory, args.src_dict_file), args.use_default_dict)
	tgtDict = getEmbeddingFromFile(os.path.join(args.directory, args.tgt_dict_file), args.use_default_dict)
...
def createSession(args, embedding):
	...
	inputVector = tf.nn.embedding_lookup(srcEmbeddingVector, input, name="encoderInput")
	...
	outputVector = tf.nn.embedding_lookup(tgtEmbeddingVector, output, name="output")
	decoderInputVector = tf.nn.embedding_lookup(tgtEmbeddingVector, decoderInput, name="decoderInput")
	...
	globalStep = tf.Variable(args.global_steps)
	args.global_steps = args.global_steps, globalStep
	#trainingTrainOp = builder.createOptimizer({'loss':loss[0], 'mode':'sgd', 'trainingRate':1.0, 'warmupTraining':(5,50), 'globalStep':globalStep })
	trainingTrainOp = builder.createOptimizer({'loss':loss[0], 'mode':'adam', 'trainingRate':0.001})
	inferTrainOp = builder.createOptimizer({'loss':loss[1], 'mode':'adam'})
	# initiate the session
	session.run(tf.global_variables_initializer())
	
	return session, [input, output, decoderInput], [batchSize, outputLengthList, maximumUnrolling, logits], [[loss[0], trainingTrainOp], [loss[1], inferTrainOp]]
	
def changeGlobalStep(args, modifier=1):
	if(len(args.global_steps) != 2):
		return
	step, globalStep = args.global_steps
	step += modifier
	globalStep.assign(globalStep + modifier)
	
def debugSession(session, feed_dict):
	outputVector = tf.get_default_graph().get_tensor_by_name("output:0")
	encoderInputVector = tf.get_default_graph().get_tensor_by_name("encoderInput:0")
	decoderInputVector = tf.get_default_graph().get_tensor_by_name("decoderInput:0")
	subtractVector = tf.get_default_graph().get_tensor_by_name("subtract:0")
	allValues = session.run([outputVector, encoderInputVector, decoderInputVector, subtractVector], feed_dict=feed_dict)
	for vectorList in allValues:
		vectorList = [vectorList[0], vectorList[6]]
		for vector in vectorList:
			print(vector)
		print('-------')
	
def trainSessionOneBatch(args, sessionTuple, batch, selector=True):
	session, inputOutputTuple, configTuple, trainTuple = sessionTuple
	trainTuple = trainTuple[0 if selector else 1]
...
def trainSession(args, sessionTuple, batches, evaluationFunction, selector=False):
	session, inputOutputTuple, configTuple, trainTuple = sessionTuple
	input, output, decoderInput = inputOutputTuple
	batchSize, outputLengthList, maximumUnrolling, _ = configTuple
	for step in range(args.epoch):
		# args.global_steps += 1
		changeGlobalStep(args)
		avgLosses = [0]
		for batch in batches:
			feed_dict = {input:batch[0], output:batch[1], batchSize:batch[2], outputLengthList:batch[3], maximumUnrolling:max(batch[3]), decoderInput:batch[4]}
			loss, _ = session.run(trainTuple[0 if selector else 1], feed_dict=feed_dict)
			avgLosses[-1] += loss
		...
		if(evaluationFunction and (step+1) % args.evaluation_step == 0):
			...
			selector = evaluationFunction((step+1,avgLosses))
		avgLosses.append(0)
	return avgLosses
...
def evaluateSession(args, session, dictTuple, sampleBatch):
	...
	inferLogits, trainLogits = logits
	feed_dict = {input:sampleBatch[0], output:sampleBatch[1], batchSize:sampleBatch[2], outputLengthList:sampleBatch[3], maximumUnrolling:max(sampleBatch[3]), decoderInput:sampleBatch[4]}
	resultInfer, resultTrain = session.run(logits, feed_dict=feed_dict)
	# _, savedData = getWordIdFromVectors(resultInfer[0], tgtEmbeddingVector, args.use_default_dict)
	#print(resultInfer, resultTrain)
	resultInfer = [getWordIdFromVectors(result, tgtEmbeddingVector, args.use_default_dict) for result in resultInfer]
	resultTrain = [getWordIdFromVectors(result, tgtEmbeddingVector, args.use_default_dict) for result in resultTrain]
	# debugSession(session, feed_dict)
	return resultInfer, resultTrain
...
if __name__ == "__main__":
	...
	if(args.save):
		...
		builder.loadFromPath(session, savePath)
		if(len(args.global_steps) == 2):
			# Use global step, check and refeed if wrong
			graphGlobalStep = session.run(args.global_steps[1])
			if(graphGlobalStep != args.global_steps[0]):
				print("Global step received in graph: %d, synchronizing with current (%d)" % (args.global_steps[0], graphGlobalStep))
				args.global_steps = graphGlobalStep, args.global_steps[1]
	...
	if(args.mode == 'train'):
		...
		def evaluationFunction(extraArgs):
			...
			inferResult, trainResult = evaluateSession(args, sessionTuple, embeddingTuple, sample)
			_, correctOutput, _, trimLength, _ = sample
			print(inferResult[0], '\n', trainResult[0], '\n', correctOutput[0])
			print(inferResult[6], '\n', trainResult[6], '\n', correctOutput[6])
			...
			print("Iteration %d, time passed %.2f, BLEU score (%2.2f, %2.2f)" % (iteration, getTimer(), inferResult * 100.0, trainResult * 100.0))
			...
			return True # trainResult < inferResult
		# execute training
